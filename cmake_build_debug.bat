@echo off
rem Полная сборка проекта Stuff (Debug) и прогон тестов.
...
cmake -S "%SCRIPT_DIR%src" -B "%SCRIPT_DIR%winbuild" -DCMAKE_BUILD_TYPE=Debug -Dblib_link_type=blib_link_static ^
    -DBUILD_TESTS=ON ^
...
echo [2/3] Building...
cmake --build "%SCRIPT_DIR%winbuild" --config Debug
if errorlevel 1 (
