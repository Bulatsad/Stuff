...
Шпаргалки по модулям лежат рядом с кодом (имя файла = имя модуля), содержат инварианты, поток данных, подводные камни и TODO. Не дублируют AGENTS.md (правила) и ARCHITECTURE.md (roadmap):
...
### Документация модулей (СТРОГО)
- При любом изменении кода модуля — **ОБЯЗАТЕЛЬНО проверить и обновить его `.md`-документ** (назначение, инварианты, поток данных, подводные камни, TODO). Изменение кода без валидации соответствующего дока считается незавершённой задачей.
- Если дока у модуля ещё нет, а правки затрагивают неочевидные инварианты/грабли — создать его рядом с кодом.
- Имя дока — по имени модуля: `src/beng/BENG.md`, `src/blib/graphics/GRAPHICS.md`, `src/blib/system/SYSTEM.md`.
- Секции шаблона: Назначение и границы / Ключевые файлы (навигация) / Инварианты и поток данных / Подводные камни и баги / TODO / Связанные доки.
- Доки не дублируют AGENTS.md (правила) и ARCHITECTURE.md (roadmap) — только ссылаются.
- Индекс доков — раздел «📖 Модульная документация» выше; обновлять при создании нового дока.
