...
{
    this->size = ((size_t)((double)fmt.sampleRate / (double)1000) * (fmt.bitRate / 8)) * fmt.channel;
...
template<class Allocator>
__blib_inline const size_t blib::SoundBufferTemplate<Allocator>::getPayloadSize() const
{
    return this->payloadSize;
}

template<class Allocator>
__blib_inline const size_t blib::SoundBufferTemplate<Allocator>::getSize() const
{
    return this->size;
}