...
namespace memory
{
    ...
    public:
        ...
        /**
         * Move-конструктор - переносит владение хранимым объектом.
         * 
         * Стратегия переноса:
         * - heap-объект: переносим только указатель heapPtr (объект физически не двигается)
         * - inline-объект: побайтово копируем buffer (объект "переезжает" в новый буфер)
         * 
         * ВАЖНО (инвариант bytewise-переноса):
         * - Деструктор исходного SBO НЕ вызывается для перенесённого объекта,
         *   поэтому не происходит ни double-destruction, ни double-free внутренних
         *   ресурсов хранимого объекта
         * - Допустимо только для объектов без внутренних указателей на самих себя
         *   (AllocatorImplWrapper удовлетворяет этому требованию: все указатели
         *   указывают на внешнюю heap-память, которая остаётся валидной)
         */
        SBO(SBO&& other) noexcept
            : heapPtr(other.heapPtr)
        {
            // Если объект жил в inline-буфере - переносим его байты в свой буфер
            if (!other.heapPtr)
            {
        ...
        /**
         * Move-оператор присваивания - симметричен move-конструктору.
         * 
         * ВАЖНО: Вызывающий код (Allocator) должен сам разрушить свой текущий
         * объект (destroyImpl) ДО присваивания, т.к. SBO не знает тип объекта
         * и не может вызвать его деструктор.
         */
        SBO& operator=(SBO&& other) noexcept
        {
            if (this != &other)
            {
                // Забираем владение у other
                heapPtr = other.heapPtr;

                // Inline-объект переносим побайтово
                if (!other.heapPtr)
                {
                    std::memcpy(buffer, other.buffer, BufferSize);
                }
        ...
        /**
         * Проверить, хранится ли объект в heap (был создан через constructInHeap).
         * 
         * @return true если объект в heap
         */
        bool isHeap() const
        {
            return heapPtr != nullptr;
        ...
        /**
         * Сбросить признак владения heap-объектом БЕЗ разрушения объекта
         * и БЕЗ освобождения памяти.
         * 
         * Использование: Allocator::destroyImpl() самостоятельно вызывает
         * деструктор объекта и GlobalAllocator::deallocate(), после чего
         * сообщает SBO что объект больше не хранится.
         */
        void releaseHeap()
        {
            heapPtr = nullptr;
