...
#include <blib/system/memory/sbo.h>

// Включаем определение AllocatorImplWrapper перед использованием
#include <blib/system/memory/impl/allocatorImplWrapper.h>
...
namespace memory
{
     ...
     */
    class __blib_system_api Allocator
    {
    public:
        /**
         ...
         * 
         * ВАЖНО: enable_if исключает сам тип Allocator из этого конструктора,
         * иначе для не-const lvalue (Allocator copy(orig)) шаблон перехватил бы
         * вызов у copy-конструктора и создал обёртку над ССЫЛКОЙ на orig
         * (dangling при смерти оригинала). Копирование Allocator всегда
         * должно идти через Allocator(const Allocator&).
         */
        template<typename AllocatorImpl,
                 typename = typename std::enable_if<!std::is_same<typename std::decay<AllocatorImpl>::type, Allocator>::value>::type>
...
#include <blib/system/memory/impl/allocator.inl>
