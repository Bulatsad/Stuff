...
namespace memory
{
        ...
        , lock(nullptr)
    {
        ...
        // Важно: используем operator new напрямую, чтобы избежать рекурсии
        lock = new thread::RWLocker();
    }
    ...
    GlobalAllocator::~GlobalAllocator()
    {
        ...
        if (statsCollector)
        {
            delete static_cast<StatsCollector*>(statsCollector);
            statsCollector = nullptr;
        }
        ...
        if (leakTracker)
        {
            delete static_cast<LeakTracker*>(leakTracker);
            leakTracker = nullptr;
        }
    ...
    {
        // Выделяем память через системный operator new
    ...
    void GlobalAllocator::deallocate(_In void* ptr, size_t size)
    {
        ...
        // Освобождаем память через системный operator delete
        ::operator delete(ptr);
    }
    ...
    void GlobalAllocator::setLeakTrackingEnabled(bool enabled)
    {
        ...
        if (enabled && !leakTrackingEnabled)
        {
            ...
            if (!leakTracker)
            {
                // Создаём LeakTracker используя operator new напрямую
                leakTracker = new LeakTracker();
            }
