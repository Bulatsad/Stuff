@echo off
rem Сборка ядра Stuff (Debug): blib-system, blib-core и их тесты,
...
cmake -S "%SCRIPT_DIR%src" -B "%SCRIPT_DIR%winbuild" -DCMAKE_BUILD_TYPE=Debug ^
    -DBUILD_TESTS=ON ^
...
cmake --build "%SCRIPT_DIR%winbuild" --config Debug --target blib-system blib-core blib_tests
if errorlevel 1 (
...
ctest --test-dir "%SCRIPT_DIR%winbuild" -C Debug --output-on-failure
if errorlevel 1 (
